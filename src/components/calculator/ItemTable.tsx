...
function ItemRow({ item, isReadOnly, onValueChange, onNameChange, onDelete }: ItemRowProps) {
    ...
    return (
        <TableRow>
            ...
            <TableCell>
                {isReadOnly ? (
                    <Input type="text" value={formattedReadOnlyValue} readOnly disabled className="bg-muted/50" />
                ) : (
                    <Input type="number" min="0" step="0.01" value={localValue} onChange={handleValueChange} placeholder="0" />
                )}
            </TableCell>
        </TableRow>
    )
